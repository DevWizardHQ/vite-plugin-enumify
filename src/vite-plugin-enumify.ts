 ...
 */
function resolveOptions(options: EnumifyOptions = {}): { options: ResolvedEnumifyOptions; config: EnumifyConfig } {
...
export function enumify(options: EnumifyOptions = {}): Plugin {
    ...
    const debouncedSync = debounce(() => {
        runSync().catch((error: Error) => logger.error(`[enumify] ${error.message}`));
    }, WATCH_DEBOUNCE_MS);
